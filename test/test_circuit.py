from qroute import environment


def test_dag_from_qasm():
    c = environment.circuits.dag_from_qasm('test/circuit_qasm/3_17_13_onlyCX.qasm')
    assert c.number_of_nodes() == 17
    assert c.number_of_edges() == 136


def test_generate_random_cirq_circuit():
    c = environment.generate.generate_random_cirq_circuit(5, 20, 0.8)
